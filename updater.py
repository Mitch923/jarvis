...
LAUNCHER_FILES = ("run.py", "updater.py", "jarvis.service")  # a broken one of these can't roll itself back
SMOKE_CODE = (
...
class Updater:
    ...
    def _env(self, auth: bool) -> dict:
        if auth and self.token and self._git_ok("remote", "get-url", "origin"):
            return git_auth_env(self.token, self._origin_url())
    ...
    def plan(self) -> Optional[Plan]:
        ...
        if not self.is_checkout():
            raise UpdateError("This folder isn't a git checkout, so I can't update myself. Clone your repo (see README).")
